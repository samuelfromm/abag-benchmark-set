...
if [ -z "$1" ] || [ -z "$2" ]; then
    echo "Usage: $0 <IDFILE> <SCOREDIR>"
    exit 1
fi

IDFILE=$1    # First argument is the path to the IDs file
SCOREDIR=$2    # Second argument is the directory containing MSA files

# Output files in the current directory
MISSINGFILE="./IDs_scores_missing.csv"
COMPLETEDFILE="./IDs_scores_completed.csv"
...
while IFS= read -r ID; do
    ...
    if [ -f "$SCOREDIR/$ID/scores.csv" ]; then
