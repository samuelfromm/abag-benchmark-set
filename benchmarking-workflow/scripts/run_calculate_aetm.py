...
def main():
    ...
    # Calculate aeTM
    aetm = aligned_error.aligned_error_confidence.compute_aetm(
        aligned_error=aligned_error_tensor
    )

    # Calculate aeiTM
    aeitm = aligned_error.aligned_error_confidence.compute_aetm(
        aligned_error=aligned_error_tensor,
        asym_id=asym_id,
        interface=True,
    )
    ...
    output_data = {
        ...
        "aetm": round(aetm.item(), precision),
        "aeitm": round(aeitm.item(), precision),
        "ae_ranking_confidence": round(ae_ranking_confidence.item(), precision),
        "max_per_chain_aeitm": round(max_per_chain_aeitm, precision),
        "min_per_chain_aeitm": round(min_per_chain_aeitm, precision),
        "mean_per_chain_aeitm": round(mean_per_chain_aeitm, precision),
    }
