...
def main():
    ...
    if "plddt" in af_data.keys():
        plddt = af_data['plddt']
    else:
        raise KeyError("Could not find the plddt data (keywords 'plddt' or 'atom_plddts').")

    if isinstance(plddt, list):
        plddt = torch.tensor(np.array(plddt), dtype=torch.float32)
    elif isinstance(plddt, np.ndarray):
        plddt = torch.tensor(plddt, dtype=torch.float32)
    elif not isinstance(plddt, torch.Tensor):
        raise TypeError("The 'plddt' value must be a list, NumPy array, or a Torch tensor.")


    # Calculate aeTM
    paetm = aligned_error.aligned_error_confidence.compute_custom_aetm(
        aligned_error=predicted_aligned_error_tensor,
        weights=plddt,
    )

    # Calculate aeiTM
    paeitm = aligned_error.aligned_error_confidence.compute_custom_aetm(
        aligned_error=predicted_aligned_error_tensor,
    ...
    output_data.update(
        {
            "custom_paetm": round(paetm.item(), precision),
            "custom_paeitm": round(paeitm.item(), precision),
            "custom_pae_ranking_confidence": round(pae_ranking_confidence.item(), precision),
            "max_per_chain_custom_paeitm": round(max_per_chain_aeitm, precision),
            "min_per_chain_custom_paeitm": round(min_per_chain_aeitm, precision),
            "mean_per_chain_custom_paeitm": round(mean_per_chain_aeitm, precision),
        }
    )
