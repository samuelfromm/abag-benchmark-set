...
rule run_calculate_actifptm:
    input:
        af_data=lambda wildcards: get_sample_value(wildcards, 'query_af_data'),
        af_features=lambda wildcards: get_sample_value(wildcards, 'query_af_features'),
    output:
        temp(output_dir+"/{sample_id}/{sample_id}_actifptm.csv"),
    conda:
        config["run_calculate_actifptm_env"]
    shell:
        """
        python scripts/run_calculate_actifptm.py \
        --sample_id {wildcards.sample_id} \
        --af_data {input.af_data} \
        --af_features {input.af_features} \
        --output_csv {output[0]}
...
rule merge_scores:
    input:
        ...
        output_dir+"/{sample_id}/{sample_id}_actifptm.csv",
    output:
