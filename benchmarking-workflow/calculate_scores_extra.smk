...
rule run_abag_ipsae:
    input:
        ...
        af_data=lambda wildcards: get_sample_value(wildcards, 'query_af_data'),
    output:
        output_dir+"/{sample_id}/ipsae/{sample_id}_query_abag_merged_10_10.txt",
        temp(output_dir+"/{sample_id}/ipsae/{sample_id}_query_abag_merged_10_10.pml"),
        temp(output_dir+"/{sample_id}/ipsae/{sample_id}_query_abag_merged_10_10_byres.txt"),
        outdir=directory(output_dir+"/{sample_id}/ipsae/"),
    conda:
        config["run_get_af_prediction_env"]
    shell:
        """
        python scripts/IPSAE/ipsae.py {input.af_data} {input.query_abag_merged} 10 10  {output.outdir}
        """

rule collect_abag_ipsae:
    input:
        abag_ipsae=output_dir+"/{sample_id}/ipsae/{sample_id}_query_abag_merged_10_10.txt",
    output:
        temp(output_dir+"/{sample_id}/{sample_id}_abag_ipsae.csv"),
    conda:
        config["run_get_af_prediction_env"]
    shell:
        """
        python scripts/collect_abag_ipsae.py \
            --sample_id {wildcards.sample_id} \
            --abag_ipsae {input.abag_ipsae} \
            --output_csv {output[0]}
        """
...
rule merge_scores:
    input:
        ...
        output_dir+"/{sample_id}/{sample_id}_abag_ipsae.csv",
    output:
